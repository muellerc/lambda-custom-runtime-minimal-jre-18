...
#   1. use the latest Amazon Linux 2 image and install an early access version of OpenJDK 18
...
docker build --progress=plain -t lambda-custom-runtime-minimal-jre-18 .

# extract the runtime.zip from the Docker container and store it locally
docker run --rm --entrypoint cat lambda-custom-runtime-minimal-jre-18 runtime.zip > runtime.zip
